...
// #define car_Sensor 3     //Sensor on Pin 3 for Car count - INTERRUPT PIN
const byte car_count_ISR = 3; // Sensor on Pin 3 for Car count - INTERRUPT PIN
...
volatile byte car_count = 0; // Count variable for car_count_ISR
...
byte car_interrupt(){
  if (car_count < 4){
    car_count += car_count; // count car's
  }
  else car_count = 0;
...
void setup() {
  ...
  pinMode(car_count_ISR, INPUT_PULLUP); // Pin 3 - Interrupt Pin
  attachInterrupt(digitalPinToInterrupt(car_count_ISR), car_interrupt, LOW);
...
car_count += digitalRead(car_count_ISR);
...
// Optical Sensor pin definition:
#define optic_Sensor_1 2 //optical Interrupt Sensor on Pin 2 - start position
#define car_Sensor 3     //Sensor on Pin 3 for Car count - FLYT TIL INTERRUPT PIN
...
void setup() {
  ...
  pinMode(optic_Sensor_1, INPUT_PULLUP); // pin 2 - internal PULLUP resistor
  pinMode(car_Sensor, INPUT); // Pin 3 - NOTE: kan bruges, hvis detektering af Faller-Car ønskes
